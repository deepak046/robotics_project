...
class detect_cube(pt.behaviour.Behaviour):
     ...
     def __init__(self):
         ...
         self.aruco_pose_top = rospy.get_param(rospy.get_name() + '/aruco_pose_topic')
             # Subscriber to get pose of the cube
         self.aruco_pose_sub = rospy.Subscriber(self.aruco_pose_top, PoseStamped, self.aruco_pose_cb)
     ...
     def aruco_pose_cb(self, pose_msg):
         if pose_msg != None:
     ...
     def update(self):
        ...
        if self.aruco_pose_rcv:
            return pt.common.Status.SUCCESS
        # if no cube is detected, return failure
        elif not self.aruco_pose_rcv:
            return pt.common.Status.FAILURE
